...
This specification describes the file formats, queries, and processes required for the use of TUF in registries. It requires that metadata for each of the TUF roles: root, snapshot, timestamp, and targets is uploaded to a registry and can be queried by users. Details about these roles can be found in the [design document](https://github.com/notaryproject/nv2/blob/prototype-tuf/tuf-design.md) and the [TUF specification](https://github.com/theupdateframework/specification/blob/master/tuf-spec.md#the-update-framework-specification).
...
* Generate and sign targets metadata by running `nv2 sign imagename keyloc --role targetsrolename` where `imagename` is the manifest they are signing, `targetsrolename` is the name of the top-level or delegated targets role the developer is a part of, and `keyloc` is a path to the developer's private key.
...
* To add a new delegatee they run `nv2 delegate fromrole torole --namespace ns --key keyloc`. This should add `torole` as a delegation to `fromrole` that is trusted for images in the namespace `ns`.
* To remove a delegatee they run `nv2 revoke fromrole torole --key keyloc`.
* After performing the above commands any number of times, they run `nv2 upload fromrole`. This will upload the new `fromrole` metadata. Any new delegated roles should sign and upload their targets metadata as well.
...
* Receive new metadata version. A developer who plans to upload new TUF targets metadata must send information about the version number of this new metadata to the snapshot process. The process may perform any verification to ensure the developers are authorized to upload a new version.
* Add metadata version to snapshot. The version number of the new metadata must be added to snapshot and associated with the correct metadata file.
...
Changes to snapshot may be batched, so multiple metadata versions may be added at once. To do so, the process could allow steps 1 and 2 to repeat any number of times before signing the snapshot metadata.
## User download
...
* Root metadata. The client should download the most recent root metadata, as well as previous versions until their currently trusted root version.
* The most recent timestamp metadata.
* The most recent snapshot metadata.
* the most recent top-level targets metadata.
* Relevant delegated targets metadata. If the top-level targets metadata delegated the queried namespace to another targets metadata file, that file should be downloaded. There may be multiple levels of delegated targets metadata.
* The requested image.
...
## Garbage Collection
Old versions of snapshot, timestamp, targets can be garbage collected anytime after the new versions are uploaded.

Old versions of root should stay on the registry to allow for users with older root keys to verify the rotation of root keys.
...
* `ROLE` is a dictionary whose "_type" field describes the role type, which will be one of `root`, `timestamp`, `snapshot`, or `targets`.
...
* `ROLE` is one of "root", "snapshot", "targets", or "timestamp". A role for each of "root", "snapshot", "timestamp", and "targets" MUST be specified in the key list.
...
* `THRESHOLD` is an integer number of keys of that role whose signatures are required in order to consider a file as being properly signed by that role.
