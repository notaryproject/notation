module github.com/notaryproject/notation

go 1.18

require (
	github.com/distribution/distribution/v3 v3.0.0-20220729163034-26163d82560f
	github.com/docker/docker-credential-helpers v0.6.4
	github.com/notaryproject/notation-core-go v0.0.0-20220831010912-8681e4f21c63
	github.com/notaryproject/notation-go v0.10.0-alpha.3.0.20220829033635-a237dc31dbf4
	github.com/opencontainers/go-digest v1.0.0
	github.com/spf13/cobra v1.5.0
	github.com/spf13/pflag v1.0.5
	oras.land/oras-go/v2 v2.0.0-rc.2
)

require (
	github.com/fxamacker/cbor/v2 v2.4.0 // indirect
	github.com/golang-jwt/jwt/v4 v4.4.2 // indirect
	github.com/inconshreveable/mousetrap v1.0.0 // indirect
	github.com/opencontainers/distribution-spec/specs-go v0.0.0-20220620172159-4ab4752c3b86 // indirect
	github.com/opencontainers/image-spec v1.0.3-0.20211202183452-c5a74bcca799 // indirect
	github.com/oras-project/artifacts-spec v1.0.0-rc.2 // indirect
	github.com/veraison/go-cose v1.0.0-rc.1.0.20220824135457-9d2fab636b83 // indirect
	github.com/x448/float16 v0.8.4 // indirect
	golang.org/x/sync v0.0.0-20220722155255-886fb9371eb4 // indirect
	golang.org/x/sys v0.0.0-20220731174439-a90be440212d // indirect
)
