...
func runImport(command *cobra.Command, opts importOpts) error {
	...
	if !opts.force {
		...
		fmt.Fprintln(os.Stderr, "Warning: existing OCI trust policy file will be overwritten")
	}
	...
	_, err = fmt.Fprintln(os.Stdout, "Successfully imported OCI trust policy file to", policyPath)
	return err
}
