...
func runSign(command *cobra.Command, cmdOpts *signOpts) error {
	...
	if err != nil {
		...
		if errors.As(err, &errorPushSignatureFailed) && !ociImageManifest {
			return fmt.Errorf("%v. Possible reason: target registry does not support OCI artifact manifest. Try using flag `--signature-manifest image` to store signatures using OCI image manifest for backwards compatibility", err)
		}
		return err
	}
